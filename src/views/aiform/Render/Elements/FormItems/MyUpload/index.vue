<template>
  <div style="padding: 0px 0px 8px 8px; width: 100%">
    <el-table
      v-if="state.fileListData.length > 0"
      v-loading="state.loading"
      :data="state.fileListData"
      row-key="id"
      style="width: 100%"
      :show-header="false"
    >
          ...
          <div class="my-flex">
            ...
            <div class="ml10 my-flex-fill my-flex-y-center">
              <el-link
                ...
                :underline="false"
                target="_blank"
                >{{ (row.fileName || "") + (row.extension || "") }}</el-link
              >
              {{ row.sizeFormat }}
            </div>
          </div>
...
const state = reactive({
  loading: false,
  fileFormTitle: "",
  fileListData: JSON.parse(JSON.stringify(props.modelValue)) || [],
  fileLogsTitle: "",
});
const onOk = (file) => {
...
const previewImglist = computed(() => {
  ...
  state.fileListData.forEach((a) => {
    if (isImage(a.extension) && a.linkUrl) {
      imgList.push(a.linkUrl);
    }
  });
  return imgList;
});
