        ...
        <template #dropdown>
          <el-dropdown-menu>
            <el-dropdown-item command="insertLeftCol"
              >向左插入1列</el-dropdown-item
            >
            <el-dropdown-item command="insertRightCol"
              >向右插入1列</el-dropdown-item
            >
            <el-dropdown-item command="insertAboveRow"
              >向上插入1行</el-dropdown-item
            >
            <el-dropdown-item command="insertBelowRow"
              >向下插入1行</el-dropdown-item
            >
            <el-dropdown-item
              command="mergeRightCol"
              :disabled="!canMergeRightCol"
              divided
              >合并右侧单元格</el-dropdown-item
            >
            <el-dropdown-item
              command="mergeBelowRow"
              :disabled="!canMergeBelowRow"
              >合并下方单元格</el-dropdown-item
            >
            <el-dropdown-item
              command="mergeWholeRow"
              :disabled="!canbMergeWholeRow"
              >合并整行</el-dropdown-item
            >
            <el-dropdown-item
              command="mergeWholeCol"
              :disabled="!canbMergeWholeCol"
              >合并整列</el-dropdown-item
            >
            <el-dropdown-item
              command="splitMerged"
              :disabled="!canSplitMerged"
              divided
              >拆分成单元格</el-dropdown-item
            >
            <el-dropdown-item
              command="deleteWholeRow"
              :disabled="!canbMergeWholeRow"
              divided
              >删除整行</el-dropdown-item
            >
            <el-dropdown-item
              command="deleteWholeCol"
              :disabled="!canbMergeWholeCol"
              >删除整列</el-dropdown-item
            >
          </el-dropdown-menu>
        </template>
      </el-dropdown>
...
const canMergeRightCol = computed(() => {
  ...
  if (leftCell.colSpan + props.colIndex >= rows[props.rowIndex].children.length)
    return false;
  var rightCell =
    rows[props.rowIndex].children[props.colIndex + leftCell.colSpan];
...
const canbMergeWholeCol = computed(() => {
  ...
  for (var k = 0; k < rows.length; k++) {
    ...
    if ((cell.state & 4) !== (rows[0].children[props.colIndex].state & 4))
      return false;
  }
  return true;
});
...
const canbMergeWholeRow = computed(() => {
  ...
  for (var k = 1; k < row.children.length; k++) {
    if ((row.children[k].state & 2) !== (row.children[0].state & 2))
      return false;
  }
  return true;
});
