...
import * as basics from "./common.js";
///定制组件
...
const getConfigByTag = (targetList, tag) =>
  targetList.find((t) => t.tag === tag);
...
export const customGroupComponents = [
  copyConfigAsCustom({ label: "请假组件", explain: null }, [
    {
      target: getConfigByTag(basics.selectComponents, "my-select"),
      config: {
        label: "请假类型",
        required: true,
        options: [
    ...
    {
      target: getConfigByTag(basics.selectComponents, "ai-date-duration"),
      config: {
        required: true,
        label: "日期范围",
        showDuration: true,
