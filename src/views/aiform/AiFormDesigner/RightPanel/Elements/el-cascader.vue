...
const remove = (node, data) => {
  ...
  const index = children.findIndex(
    (d) => (!!d.id && d.id === data.id) || d.value === data.value
  );
...
const renderContent = (h, { node, data, store }) => {
  ...
  return h("div", { class: "custom-tree-node" }, [
    ...
    h("span", { class: "node-operation" }, [
      h(ElIcon, { onClick: () => append(data) }, [h(resolveComponent("Plus"))]),
      h(ElIcon, { onClick: () => remove(node, data) }, [
  ...
  //             <el-icon onClick={() => this.append(data)} title="添加" ><Plus  /></el-icon>
  //             <el-icon onClick={() => this.remove(node, data)} title="删除" ><Delete /></el-icon>
