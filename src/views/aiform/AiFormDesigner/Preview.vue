    ...
    <el-drawer v-model="state.isShow" direction="rtl" :size="state.drawerSize">
      <template #header="{ titleId, titleClass }">
        <h4 :id="titleId" :class="titleClass">
          {{ state.conf?.formConf?.title }}
        </h4>
        <el-button-group>
          <el-button
            @click="state.drawerSize = '390px'"
            icon="Cellphone"
            title="手机"
          >
          </el-button>
          <el-button
            @click="state.drawerSize = '100%'"
            icon="Monitor"
            title="电脑"
          ></el-button>
        </el-button-group>
      ...
      <template #footer>
        <div>
          ...
          <el-button type="info" plain @click="state.isShow = false"
            >取消</el-button
          >
        </div>
      </template>
...
const FormParser = defineAsyncComponent(() =>
  import("../AiFormParser/index.vue")
);
