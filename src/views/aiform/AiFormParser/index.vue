...
const state = reactive({
  ...
  formConf: Object.assign(
    { tableRefs: {}, model: JSON.parse(JSON.stringify(props.conf.model)) },
    props.conf.formConf || {}
  ),
...
const buildRules = (item) => {
  ...
  const rules = [];
  //如果禁用了设置为非必填
  if (item.disabled) item.required = false;
  if (item.required) {
    const type = Array.isArray(item.defaultValue) ? "array" : undefined;
    let message = Array.isArray(item.defaultValue)
      ? `请至少选择一个`
  ...
  if (item.regList && Array.isArray(item.regList)) {
    item.regList.forEach((item) => {
      if (item.pattern) {
        rules.push({
          pattern: eval(item.pattern),
          message: item.message,
          trigger: trigger[item.tag],
        });
      }
    });
  }
